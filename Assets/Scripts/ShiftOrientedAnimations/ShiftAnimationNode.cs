...
public class ShiftAnimationNode
{
    ...
    public ShiftAnimationNode(ShiftTransformNode shiftNode)
    {
        Enabled = true;
        _transform = shiftNode.Transform;
    ...
    private Vector2 GetOffset(Vector2 v, Vector2 direction)
    {
        ...
        else if (_transform.name == "Right hand" || _transform.name == "Left hand") return new Vector2(direction.x <= 0 ? v.x - 0 : v.x, v.y);
        else return v;
    }
